...
const PersonalInterviews = () => {
  ...
  useEffect(()=>{
    const fetchData = async () =>{
      try{
        const response = await axios.get("http://backend:3000/experience/personal",{withCredentials:true})
  ...
  const handleUpdateJob = async (updatedJob: InterviewDetails) => {
    try{
       if(updatedJob.interviewExperienceId){
          const response = await axios.get(`http://backend:3000/experience/update?interviewExperienceId=${updatedJob.interviewExperienceId}`,{withCredentials:true})
          ...
          await axios.post("http://backend:3000/experience/personal/update",updatedJob,{withCredentials:true});
  ...
  const handleDeleteJob = async (jobToDelete: InterviewDetails) => {
    try{
      if(jobToDelete.interviewExperienceId){
        ...
        if(!response) return;
        await axios.delete(`http://backend:3000/experience?interviewId=${jobToDelete.interviewExperienceId}`,{withCredentials:true});
      }
      else{
        await axios.delete(`http://backend:3000/experience/personal?interviewId=${jobToDelete.id}`,{withCredentials:true});
      }
  ...
  const addInterview = async (payload:{companyName:string,experienceType:string,interviewStatus:string,interviewExperienceId:undefined}) => {
    try{
     ...
     const response = await axios.post("http://backend:3000/experience/personal",{...payload,email:userDetails?.email},{withCredentials:true});
