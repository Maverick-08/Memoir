...
export const getAllInterviewExperience = async (req: Request, res: Response) => {
    try {
        ...
        res.status(StatusCode.ServerError).json({ msg: "Failed to fetch interview experiences" });
...
export const getUserInterviewExperience = async (req: Request, res: Response) => {
    try {
        // Extract the email from the request
        const email = req.userDetails?.email;

        if (!email) {
            res.status(StatusCode.Unauthorized).json({ msg: "User details not found" });
            return;
        }
        ...
        res.status(StatusCode.RequestSuccessfull).json({ data: userInterviewExperiences });
        ...
        res.status(StatusCode.ServerError).json({ msg: "Failed to fetch user interview experiences" });
    ...
    try {
        ...
        for (let i = 0; i < interviewExperience.roundDetails.length; i++) {
            if (interviewExperience.roundDetails[i].roundName === "" || interviewExperience.roundDetails[i].roundType === "") {
                res.status(StatusCode.BadRequest).json({ msg: "Round name & Round type cannot be empty" });
                return;
            }
        }
            ...
            if (round.questions) {
                for (let j = 0; j < round.questions.length; j++) {
                    if (!round.questions[j].title || !round.questions[j].description) {
                        res.status(StatusCode.BadRequest).json({ msg: "Question title or description cannot be empty" });
                        return;
                    }
                }
            }
        ...
        res.status(StatusCode.ResourceCreated).json({ msg: "Interview experience record created successfully" });
        return

    } catch (err: any) {
        console.error("Error @postInterviewExperience \n"+err);

        res.status(StatusCode.ServerError).json({ msg: "Failed to create interview experience" });
